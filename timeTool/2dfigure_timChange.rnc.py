...
def timsChoice(avg, nrolls):
	step = 2 * math.pi / nrolls;
	delta = 0.1 * step;
	
	avgNorm = np.array([val + i * step - 2 * math.pi if i * step > math.pi else val + i * step for i, val in enumerate(avg)]);
	numInBestCluster = 0;
	bestGuess = 20;
...
for lineout in lineouts:
	...
	d_data_tim = timsChoice(avg,nrolls)
	d_data_ryan = chooseslope(avg,nrolls)
						
	D = np.row_stack((D,d_data_tim))
	DR = np.row_stack((DR,d_data_ryan))

print 'Tim\'s RMSE' 
print math.sqrt(mse(ground_truths, D))
print 'Ryan\'s RMSE'
print math.sqrt(mse(ground_truths,DR))

dirstr = 'data_simulation/delays/'
filename_tim=dirstr + 'tims_delays.dat'
filename_ryan=dirstr + 'ryans_delays.dat'
np.savetxt(filename_tim,D,fmt='%.6e')
np.savetxt(filename_ryan,DR,fmt='%.6e')
