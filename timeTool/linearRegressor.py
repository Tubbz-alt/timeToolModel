...
def read_training_data(data_dir, write_dir, run_num):
	...
	# Convert to np.array and matrix for LR
...
############################################ 
def cross_validate(partitioned_data, partitioned_truths):
        ...
        for k in range(0, 3):
                ...
                # Train and apply linear model to train & test data
                model = train(training_data, training_truths)
                y_pred = apply_linear_model(model, testing_data)
...
#########################################
#       Apply trained LR to test        #
#########################################
def apply_linear_model(model, testing_data):
...
if __name__ == "__main__":
	...
	# Set up arg parser  
        helpstr = 'Perform 3-fold cross validation for linear regressor on data in pixel_pos range [350,352)'
